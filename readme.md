...
- 容量瓶颈，由于各个进程在使用内存空间的时候中间会剩余一些空闲的内存碎片，这时候如果想加载进程3的话，那么必须有连续的空闲并且足够装载进程3，才能进行运行，即便使两段空闲内存加起来满足也不可以运行。采用页表模式可以改善这个问题，可以将内存切割为多个页表，进程占用的基本单位为页表，这样就可以更好的利用内存空间了。

![image-20250502120653220](./assets/image-20250502120653220.png)
...
如下图是一级页表的映射原理，下面三段虚内存代表三个进程，每个进程中的每一块虚拟内存对应着页表，然后每个页表页对应这一部分物理地址，只要页表的基址+虚拟内存的偏移就可以找到真实的物理地址。这样做的好处是，每个进程可以直接根据页下标就可以找到页表数据，从而计算出真实的物理地址来，但是坏处是每个进程再访问无论用没用到，都需要将整个页表缓存起来。那么可不可以将用到了的页表再加载到缓存中，答案是不太推荐，因为只缓存用到的页表的话，页与页直接的关系是不连续的，那么虚拟内存找物理内存的话，需要遍历整个页表，找到合适的页，然后加偏移才能计算出真实的物理地址，这样会提高页查找的复杂性已经查找时间，如果将整个页表全部缓存起来的换，虚拟内存直接根据下标找到页表，然后加偏移就可以定位物理地址。

![17bb98733c88b6dac8345e4ccd341976](./assets/17bb98733c88b6dac8345e4ccd341976.png)

为了解决这种浪费空间的问题，多级页表诞生了，在多级页表下，首先按块将页进行划分组成页目录，也就是下图的二级页表，会直接与一级页表做关联，一级页表也会按块来自由划分，如下图的T1和T2，一级页表会与物理内存做关联。在二级页表这种模型下，只有用到了的页目录才会加载到二级页表中，在虚拟内存寻址的时候，先根据虚拟内存地址找到二级页表然后根据二级页表加页偏移找到真实的页，然后根据页加偏移找到真实的物理地址。
